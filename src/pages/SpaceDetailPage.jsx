...
export default function SpaceDetailPage() {
    ...
    const upcomingRes = state.reservations
        .filter(r => r.id_espacio === id && r.estado === 'Aprobado' && r.fecha >= today)
            ...
            <div className="card p-6 mb-5">
                <div className="flex items-start gap-4">
                    <div className="w-14 h-14 rounded-xl bg-brand-50 flex items-center justify-center text-3xl flex-shrink-0">
                        {TYPE_ICONS[space.tipo]}
                    </div>
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                            <h1 className="font-display font-bold text-xl text-gray-900">{space.nombre}</h1>
                            {space.requiere_aprobacion && (
                                <span className="badge bg-amber-100 text-amber-700">Requiere aprobación</span>
                            )}
                        </div>
                        <p className="text-gray-500 text-sm">{space.descripcion}</p>
                    </div>
                </div>

                {/* Detalles */}
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-6 pt-5 border-t border-surface-border">
                    <Detail icon="👤" label="Capacidad" value={`${space.capacidad} personas`} />
                    <Detail icon="📍" label="Edificio" value={space.edificio} />
                    <Detail icon="🏷️" label="Tipo" value={space.tipo?.charAt(0).toUpperCase() + space.tipo?.slice(1)} />
                </div>
                    ...
                    <div className="space-y-2">
                        {upcomingRes.map(r => (
                            <div key={r.id} className="flex items-center justify-between py-2 border-b border-surface-border last:border-0">
                                <span className="text-sm text-gray-700">{r.fecha}</span>
                                <span className="text-sm font-medium text-gray-900">{r.hora_inicio} – {r.hora_fin}</span>
                            </div>
                        ))}
                    </div>
                )}
